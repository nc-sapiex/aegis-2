...
- \`tenantId\` is the tenant discriminator, read from the session only (never
  params, body, headers or query). RLS policies enforce it at the database
  layer (\`FORCE ROW LEVEL SECURITY\`, ADR 0001) via \`prismaForTenant\`, and
  **every query against a table carrying \`tenantId\` must still scope by it**
  in application code too — RLS is the backstop, not a substitute.
...
const RLS_REFERENCE_TABLES = new Set([
  "RbiCircular",
  "RbiMasterDirection",
  "RbiChecklistItem",
]);
  ...
  return models
    .filter(
      (m) =>
        ...
        !RLS_REFERENCE_TABLES.has(m.name),
    )
    .map((m) => m.name)
    .sort();
}
