...
function smtpMailer(): Mailer {
  ...
  // SMTP usernames are frequently not email addresses (an API key, an IAM
  // access key id) and can't stand as a From header. Prefer SES_FROM_EMAIL,
  // shared with the ses driver, and fall back to the username only when it's
  // unset.
  const from = process.env.SES_FROM_EMAIL ?? user;
  return {
    async send(params) {
      try {
